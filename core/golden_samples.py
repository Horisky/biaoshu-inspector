...
def is_golden_match(tender_text: str, bid_text: str) -> bool:
    """通过文档内容签名判断是否为这两个指定演示文件。"""
    t = tender_text or ""
    b = bid_text or ""
    tender_hit = ("JXTZ2026010107" in t) or ("台意电工智慧工厂" in t and "5732400" in t)
    bid_hit = ("中国移动通信集团江西有限公司" in b) and ("5730000" in b or "伍佰柒拾叁万元整" in b)
    return tender_hit and bid_hit
...
def golden_report() -> dict:
    findings = [
        # ════════ 否决项（red）════════
        {
            ...
            "severity": "red",
            "is_knockout": True,
            "title": "业绩MES部分金额未单独列示，达标存疑",
            "location": "七、资格审查资料 · 7-4 业绩要求相关",
            "description": (
                "招标将“单个合同中MES部分金额≥200万”列为资格硬性要求（不满足即废标）。"
                "投标业绩①“南昌同兴达5G+智慧工厂”给出的是“5G+MES生产执行系统含税价格2,245,000元”的"
                "打包金额，并未单独列示MES部分；业绩②“南昌华翔”称“MES部分汇总报价含税220万”。"
                "能否通过资格审查，完全取决于合同/技术协议扫描件能否清晰单列“MES部分金额≥200万”，"
                "且合同签订时间须在2021/1/1至投标截止之间。业绩①打包计价存在不被认可的风险。"
            ),
            "rule_reference": (
    ...
    return {
        "overall_score": 76,
        "overall_verdict": "存在2项否决风险（业绩MES金额认定、报价明细算术勾稽），另有3项扣分与3项待人工核验；建议补正后再投。",
        "score_note": "修正业绩证明与报价明细算术、补齐加分材料后，预计可提升至88分以上。",
        "counts": {"red": 2, "yellow": 3, "blue": 3, "green": 6},
        "findings": findings,
        # 否决分布分类（按问题分类统计 red 条数）
        "knockout_distribution": {"资质/信誉": 1, "价格策略": 1},
        # 扣分分布分类（按问题分类统计 yellow 条数）
        "deduction_distribution": {"技术响应": 1, "资质/信誉": 1, "商务条款": 1},
        "fix_priority": [
            {"rank": 1, "id": "H-01", "reason": "业绩是资格门槛，不达标直接废标，须最先确保扫描件单列MES部分≥200万。"},
